> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [blog.csdn.net](https://blog.csdn.net/deniro_li/article/details/78458306)

![](https://imgconvert.csdnimg.cn/aHR0cDovL3VwbG9hZC1pbWFnZXMuamlhbnNodS5pby91cGxvYWRfaW1hZ2VzLzMzODYxMDgtNTI0OWMyY2ZmZTkyZjJiYy5wbmc?x-oss-process=image/format,png)

网站的可扩展性[架构](https://so.csdn.net/so/search?q=%E6%9E%B6%E6%9E%84&spm=1001.2101.3001.7020)设计，能够在对现有系统影响最小的情况下，系统功能可以可持续扩展及提升的能力。
...
![](https://imgconvert.csdnimg.cn/aHR0cDovL3VwbG9hZC1pbWFnZXMuamlhbnNodS5pby91cGxvYWRfaW1hZ2VzLzMzODYxMDgtMjFhM2QwODlkZjQ5NWM3Yy5wbmc?x-oss-process=image/format,png)
...
![](https://imgconvert.csdnimg.cn/aHR0cDovL3VwbG9hZC1pbWFnZXMuamlhbnNodS5pby91cGxvYWRfaW1hZ2VzLzMzODYxMDgtNGEwM2ZmNjU1ZDZhODgyOS5wbmc?x-oss-process=image/format,png)

目前较为流行的分布式消息队列是 Apache ActiveMQ。

因为消息队列服务器上的数据可以看做是即时处理的，所以在**伸缩性**上，我们把新服务器加入分布式消息队列集群后，只需要通知生产者服务器更改消息队列的服务器列表就好啦 O(∩_∩)O~
...
分布式消息队列可以很复杂，比如支持 ESB（企业服务总线）和 SOA（面向服务的架构）等。也可以很简单，比如使用 MySQL 作为分布式消息队列：消息的生产者把消息作为记录写入数据库，消费者查询数据库（按记录写入库表的时间戳排序），这就是一个分布式消息队列啦。再配上成熟的 MySQL 运维手段，也可以达到一个较高的可用性和性能指标哦 O(∩_∩)O~
...
*   新增业务困难。在这样一个剪不断、理还乱的系统中新增业务？开玩笑吧 O(∩_∩)O~
...
![](https://imgconvert.csdnimg.cn/aHR0cDovL3VwbG9hZC1pbWFnZXMuamlhbnNodS5pby91cGxvYWRfaW1hZ2VzLzMzODYxMDgtOWNiMzE1OWZlNWU0NzNmYy5wbmc?x-oss-process=image/format,png)
...
![](https://imgconvert.csdnimg.cn/aHR0cDovL3VwbG9hZC1pbWFnZXMuamlhbnNodS5pby91cGxvYWRfaW1hZ2VzLzMzODYxMDgtNDM5MDY4MWJlMTcwMmM4Yi5wbmc?x-oss-process=image/format,png)
...
![](https://imgconvert.csdnimg.cn/aHR0cDovL3VwbG9hZC1pbWFnZXMuamlhbnNodS5pby91cGxvYWRfaW1hZ2VzLzMzODYxMDgtYjQxZmVhYmU2NmE1ZjVhNy5wbmc?x-oss-process=image/format,png)
...
![](https://imgconvert.csdnimg.cn/aHR0cDovL3VwbG9hZC1pbWFnZXMuamlhbnNodS5pby91cGxvYWRfaW1hZ2VzLzMzODYxMDgtYWQxY2Y4M2Q4N2M3Mzc3MS5wbmc?x-oss-process=image/format,png)
